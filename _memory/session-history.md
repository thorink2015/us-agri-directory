...
## 2026-05-02 — Romanian remnants cleanup + phase A follow-up audit (branch claude/audit-public-content-w2Iz1)

- **PR #89 (merged):** audit-only pass cataloging every leftover TerraDron.ro / DroneAgricol.ro reference still on the public surface. Output at `audit/romanian-content-audit.md`. Confirmed clean: `llms.txt`, `llms-full.txt`, `ads.txt`, IndexNow key file, dynamic robots/sitemap, all `src/` source. Confirmed dirty: `humans.txt`, `.well-known/security.txt`, `public/images/README.md`, plus boilerplate gaps in `package.json` and root `README.md`.
- **PR #90 (open, draft):** two commits.
  - `fix: replace Romanian content remnants in public files + project meta` — rewrote `humans.txt` and `.well-known/security.txt` (contact `eugen@agdronedirectory.com` from `src/data/author.ts`, expires 2027-05-02), stripped DroneAgricol.ro brand and Romanian alt-tag guidance from `public/images/README.md`, renamed `package.json` to `ag-drone-directory` with description, replaced `create-next-app` boilerplate in `README.md` with real project intro + tech stack + dev commands.
  - `chore: phase A follow-up audit (author, thin pages, 404s)` — wrote `audit/phase-a-followup-audit.md`. Findings: author file fully populated (avatar resolves, schema clean); 3 thin operator profiles (~50–100 unique words each) compared against rich `agriforce-drone` (~350+ words) with 7 template-level fixes that lift the cluster without per-operator outreach; `/regions/delta` is a ghost URL (zero internal links — recommend 301 → `/regions/mississippi-delta`); `/drones/dji-agras-t25p` is referenced by 27 operator records but has no `DroneModel` entry (recommend create the catalog entry); `/newsletter` is a real homepage form-action 404 (recommend repoint to Formspree, not create the page); `/states/mississippi/crops/corn` is a state-crop combo template generating 408 near-duplicate URLs from the cartesian product, ~280 words each (recommend threading `crop.longDescription`, `crop.faqs`, `state.sprayWindows` and licensing into the template, plus gating weak combos); the 2026 statistics guide is a 3,800-word schema-rich page that's six days old — most likely indexation lag, no code change warranted yet.
- **Override called out in PR #90:** instructions said `/newsletter` was a 404 to "create vs kill." Actually it's a live homepage form action (`src/app/page.tsx:624`) — recommended fixing the form action rather than creating an empty page or removing the homepage signup.

## 2026-05-02 — Three 404 fixes (branch claude/audit-public-content-w2Iz1, PR #91)

- **Trigger:** `audit/phase-a-followup-audit.md` (PR #90) flagged three real 404s — `/regions/delta`, `/drones/dji-agras-t25p`, `/newsletter` — and the user requested independent fixes for each.
- **Commit 1 — `fix(redirects): 301 /regions/delta to /regions/mississippi-delta`:** added a `force=true` 301 to the existing operator-slug redirect block in `netlify.toml`. Verified `mississippi-delta` is the canonical region slug in `src/data/regions.ts`.
- **Commit 2 — `feat(catalog): add DJI Agras T25P drone profile`:** new full `DroneModel` entry in `src/data/drone-model.ts`, modelled exactly on the `dji-agras-t25` shape. Specs mirror the global T25 (DJI has not comprehensively published differentiated US-only specs; this caveat is stated in description, longDescription and aeoBlock so the page is honest per `_memory/copy-source-of-truth.md`). NDAA non-compliance called out 4 times. 6 FAQs (NDAA status, Part 137 + Section 44807, T25 vs T25P, 2026 pricing, T25P vs T50, where to buy). 5 authority links (DJI Agriculture US, T25 spec sheet baseline, dealer locator, FAR 52.204-25, FAA Part 137). Removed the now-redundant `dji-agras-t25p` fallback label from `DRONE_NAME_FALLBACKS`. 27 operator records that reference the slug now resolve to a real profile.
...
## 2026-05-02 — State-crop template uplift (branch claude/audit-public-content-w2Iz1, PR #94)

- **Trigger:** `audit/phase-a-followup-audit.md §2.4` (PR #90) flagged the state-crop matrix as 400 near-duplicate pages with ~280 unique words each. Same helper-module pattern as the operator uplift.
- **Commit 1 — `feat(lib): state-crop-content helpers for template-level enrichment`:** new pure-helpers module `src/lib/state-crop-content.ts`. Exports `findCropSprayWindows` (substring matches free-form sprayWindows[].crop strings against a crop slug via a per-crop keyword map: `corn`→[corn, maize], `grapes`→[grape, vineyard, wine], etc.), `getStateTopCropEntry`, `composeStateCropIntro` (90–140 words, conditional sentences), `composeStateCropFAQs` (7–11 FAQs), `stateCropFAQSchema`, `shouldNoindexStateCrop`, `getNoindexBreakdown`.
- **Commit 2 — `feat(state-crops): wire content-enrichment helpers + noindex gate into template`:** 6 new/rewired sections in `src/app/states/[slug]/crops/[crop]/page.tsx`. Region label cross-links to `/regions/[slug]` when state.regionSlug exists. State-specific intro paragraph below H1. Spray-window callout listing every matching window (Texas/cotton renders both insecticide and defoliant). Crop info card now uses `crop.longDescription` (150–250 words) instead of `crop.description` (40 words). New state licensing block. Combined 7–11 FAQ block + FAQPage JSON-LD. Noindex gate emitted via metadata `robots: { index: false, follow: true }` for weak combos (URLs stay buildable / linkable, just stop competing for crawl budget). `generateStaticParams` logs the gate distribution once per build.
- **Verified word counts** (rendered HTML, full main column): mississippi/corn 1934→2753, iowa/corn 774→1656, texas/cotton 834→1732 — all clear the 700-word target. **99 of 400 combos** correctly emit `<meta robots="noindex,follow">` after a clean rebuild. Distribution: corn=5, soybeans=9, wheat=2, cotton=24, rice=28, grapes=23, orchards=8, cover-crops=0 (mostly Alaska/Hawaii and Northeast states for crops with no in-state production base).
...
## 2026-05-02 — City seed data (branch claude/audit-public-content-w2Iz1, PR #98)

- **Trigger:** `audit/city-pages-feasibility.md` (PR #95) recommended layering USDA NASS + Census Places seed data on top of the operator-derived city set. Phase 1 inspection confirmed the existing `/states/[slug]/[city]` template at 653 lines was already rich (4 JSON-LD schemas, AEO block, services breakdown, crops table, FAQ accordion, 'how to hire' 3-step section, AuthorCard) — no template uplift needed beyond minor 0-operator handling.
- **Commit 1 — `feat(data): seed 196 ag-relevant US cities from USDA NASS + Census Places`:** new `src/data/seed-cities.ts` with 196 hand-curated entries across 48 states (skipped DC; Nevada has no defensible US ag base + 0 operators). Per-state cap of 5; lower counts for weak-ag states. Each entry: stateSlug, name, county, population (where Census-verified), agNote (1-2 sentences of ag-relevance copy from primary references). 81% operator-city population rate (319/392) confirmed in audit; 34 of the 196 seeds have at least 1 in-state operator who lists that city.
- **Commit 2 — `feat(cities): merge seed cities into the city index`:** two-pass `buildCityIndex()` in `src/data/cities.ts`. Pass 1 keeps the existing operator-derived path; pass 2 merges seeded entries (overwriting metadata only when missing, never the operator list). Extended `CityData` with optional `isSeed`, `county`, `population`, `agNote`. Qualifying-cities test now accepts a city if it EITHER hits the operator-count threshold OR is explicitly seeded.
- **Commit 3 — `feat(city-template): graceful 0-operator handling + noindex gate`:** template adjustments for seeded zero-op cities. AEO opener and FAQ generator branch on `operators.length`, statewide operator grid as fallback (up to 12 cards), crops table falls back to `county.mainCrops` resolved through `crops.ts`, credentials box hidden when operators.length===0, new ag-county context callout citing USDA NASS + Census Places, page-level `<meta robots="noindex,follow">` for cities that are seeded AND zero-op AND statewide-ops <3.
- **Verified word counts** (rendered HTML, full main column): iowa/ames 856 (strong), texas/lubbock 865 (strong), kentucky/bowling-green 863 (medium), mississippi/greenville 890 (medium), alaska/palmer 794 + noindex (weak), hawaii/hilo 825 (weak but indexable, 6 statewide ops). All 216 pages clear the 700-word target. Top 5 fattest: north-carolina/mount-olive 1748, north-carolina/greenville 1713, georgia/athens 1708, georgia/vidalia 1706, georgia/albany 1704.
...
## 2026-05-02 — Full technical SEO + AEO + performance audit (branch claude/audit-public-content-w2Iz1, PR #100)

- **Trigger:** broad audit covering schema, indexing config, meta tags, performance, AEO, internal link graph, security, on-page Ahrefs-style. 8 audit files committed; no source changes.
- **Findings:** 1 CRITICAL (sitemap submits ~334+ noindex pages), 3 HIGH (drones missing Product schema; state-operators no schema; 216 city pages no og:image), 10 MEDIUM, 15 LOW.

## 2026-05-02 — CRITICAL + HIGH fixes from PR #100 (branch claude/audit-public-content-w2Iz1, PR #101)

- **Commit 1 — `fix(sitemap): exclude ~484 noindex pages via shared gate predicates`:** new `src/lib/indexing-gates.ts` consolidates every shouldNoindex predicate site-wide (state-crop, state-service, ultra-thin-operator, state-operators, city). Both page templates and the sitemap import from this single source — no more drift between the noindex meta tag and the sitemap entry. Sitemap **1,903 → 1,419** URLs (-484 noindex'd). The 484 is higher than the audit's ~334 estimate because the audit only counted explicitly-flagged states from each PR's breakdown; the actual gate logic catches more state-service combos.
- **Commit 2 — `feat(drones): add Product schema with brand, model, offer`:** all 17 drone pages now render a Product JSON-LD with brand/manufacturer, model, additionalProperty (country, NDAA status, status, tank/swath/MTOW), and offers (when MSRP parseable from the free-form data field). MSRP digit-cluster parser handles strings like "18,000 pre-tariff; 22,000 to 28,000 post-tariff" by extracting the lowest figure. No fabricated AggregateRating or Review.
- **Commit 3 — `feat(state-operators): add CollectionPage + ItemList schema across 50 pages`:** all 50 /states/[slug]/operators pages now ship BreadcrumbList + CollectionPage(mainEntity: ItemList). Schema rendered even on the 9 PR #97-noindex'd pages — useful as a deep-link signal.
- **Commit 4 — `feat(cities): per-city dynamic og:image generation`:** new `src/app/states/[slug]/[city]/opengraph-image.tsx` route handler generates a 1200×630 PNG per qualifying city via Next.js ImageResponse. Image renders city name, state abbreviation, top crop or top service, operator count, and brand chrome. All 216 city pages now reference unique per-city og:image URLs (was 216 missing). Build-time impact ~10–15 seconds for the 216 images.
